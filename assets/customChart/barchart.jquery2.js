;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype = {
		...
		drawY: function(el, options) {
			...
			var step = Math.round(max / options.stepsCount);
			...
			while (top < options.maxHeight) {
				...
				if (gridValue <=99999) {
					gridValue = gridValue.toFixed(0);
				}else if (gridValue >= 100000 && gridValue <= 9999999) {
					gridValue = (gridValue / 100000).toFixed(0) + ' লাখ';
				} else{
                                      	gridValue = (gridValue / 10000000).toFixed(0) + ' কোটি';
                                }
				...
				y.innerHTML = '<div>' + gridValue.getDigitBanglaFromEnglish() + ' &nbsp;</div>';
		...
		drawX: function(el, options) {
			...
			for (var k in keys) {
				...
				if (keys.hasOwnProperty(k)) {
					...
					barTitle.textContent = text.getDigitBanglaFromEnglish();
					...
					barSum.textContent = (Number( localSum.toFixed(4)).toString()).getDigitBanglaFromEnglish(); // trim trailing zeros
